...
*   **What is missing:** There is no graphical interface to visualize the swarm's behavior, agent positions, pheromone trails, or claim networks.
*   **Why it matters:** Understanding the complex emergent behaviors of the swarm is difficult with only console output. Visualizations are essential for interpreting simulation results, identifying patterns, and debugging agent interactions.
*   **What blocks it:** A **capability not yet buildable**. Requires integration with a visualization library (e.g., Matplotlib, Pygame) or a web-based interface.
